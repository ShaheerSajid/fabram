...
def _library_header(macro_name: str, cfg: CharConfig) -> str:
    ...
    /* Generated by fabram characterizer */
    library ({lib_name}) {{
      technology (cmos);
...
def _sh_timing_groups(pin_name: str, sh_data: dict) -> str:
    ...
    # sh_data = {"setup": 2D-list, "hold": 2D-list}
    # Liberty convention: rise/fall constraints are for rising/falling edges of the
    # *constrained* pin, not of CLK.  For an SRAM the data-path is non-trivial
    # so we use the same measured table for both rise and fall of the constrained
    # pin (conservative, correct for synchronous cells).
